...
} from '../../../redux/features/recentEvent/recentEventApi';

const AllRecentEvent = () => {
  const { data, isLoading } = useGetRecentEventQuery();
  const [deleteRecentEventImage] = useDeleteRecentEventMutation();

  const handleDelete = async (id) => {
    try {
      const res = await deleteRecentEventImage(id).unwrap();
